...
from docutils.writers.latex2e import CharMaps, PreambleCmds
...
class XeLaTeX_TEMPLATES(object):
    ...
    * FOOTER
    ...
    HEADER = r"""%% Generated by xelatex sphinx-extension.
%% set program xelatex
\usepackage{fixltx2e}
\usepackage{fontspec}
...
# ==============================================================================
    ...
    def __call__(self):
        return (r"\documentclass[%s]{%s}"
                % (self.documentoptions, self.documentclass))

    @property
    def documentoptions(self):
        return ','.join([
            self.docCfg.paper_size
            , self.docCfg.font_size
            ])
    @property
    def documentclass(self):
        return self.docCfg.documentclass
...
# ==============================================================================
    ...
    def __init__(self):
        self.active    = set()
        self.available = dict(
            admonition         = PreambleCmds.admonition
            , align_center     = PreambleCmds.align_center
            , color            = PreambleCmds.color
            , error            = PreambleCmds.error
            , inline           = PreambleCmds.inline
            , secnumdepth      = PreambleCmds.secnumdepth
            , title            = PreambleCmds.title
            , titlereference   = PreambleCmds.titlereference
            , __static         = r'\usepackage{ifthen}'
        )

    def add(self, reqname):
        if reqname not in self.available.keys():
            raise KeyError("unknown requirement '%s'" % reqname)
        self.active.add(reqname)

    def __call__(self):
        return "\n".join([self.available[name] for name in self.active])
...
# ==============================================================================
    ...
    def visit_inline(self, node, ctx): # <span>, i.e. custom roles
        for cls in node['classes']:
            ...
            if cls == 'align-center':
                self.requirements.add('align-center')
                ctx.end_tags.push('}')
    ...
    def visit_title(self, node, ctx):
        ...
        # Table caption
            ...
            if hasattr(PreambleCmds, 'secnumdepth'):
                self.requirements.add('secnumdepth')
            ctx.body.push('\n\n')

            # System messages heading in red:
            if 'system-messages' in parent['classes']:
                self.requirements.add('color')
                ctx.body.push(
                    r'\%s[%s]{\color{red}'
                    % (self.d_class.sectionname, cmap.mask(node.astext())))
            else:
    ...
    def visit_admonition(self, node, ctx):
        self.requirements.add('admonition')
        if 'error' in node['classes']:
            self.requirements.add('error')
    ...
    def visit_title_reference(self, node, ctx):
        self.requirements.add('titlereference')
    ...
    def visit_desc_parameterlist(self, node, ctx):
    ...
    def visit_desc_parameter(self, node, ctx):
